...
def collect_kfolden_confidence_score(args, save_eval_dir, full_id_label_lst, confidence_strategy="msp"):
...
def main():
    ...
    if args.enable_leave_label_out:
        ...
        get_confidence_strategy_lst = ["msp", "temp_scaling", "mahalanobis", ""]
        ...
        for strategy_item in get_confidence_strategy_lst:
            id_pred_label_idx_array, id_confidence_array, ood_confidence_array = collect_kfolden_confidence_score(args, save_eval_dir, full_id_label_lst, confidence_strategy=strategy_item)
            id_acc = compute_accuracy(id_pred_label_idx_array, full_label_idx_lst)
            ...
            print(f"> ID-ACC : {id_acc}")
            print(f"> FPR@TPR95 : {fpr_value}")
            print(f"> AUROC : {auroc_value}")
            print(f"> ID-AUPR : {id_aupr_value}")
            print(f"> OOD-AUPR : {ood_aupr_value}")
            print(f"> MINI-OOD-ERROR : {mini_ood_error}")
            print("%%%"*60)
